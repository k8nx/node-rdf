...
class Column {
    ...
    concat(delimiter = '') {
        const func = (values) => values.reduce((prev, current) => prev + delimiter + current);
        return new Column(this.position, `concat(${this.name})`, func, true);
    }
...
class DataFrame {
    ...
    renameColumns(names) {
        const data = this._cloneMeta();
        data.meta.columns = names;
        data.rows = this.rows;
    ...
    renames(names) {
        const data = this._cloneMeta();
        data.meta.keys = names.slice(0, this.meta.keys.length);
        data.meta.columns = names.slice(this.meta.keys.length);
        data.rows = this.rows;
    ...
    sort(desc = false) {
        const data = this._cloneMeta();
        data.rows = this.rows.sort((a, b) => !desc ? a[0] > b[0] : a[0] < b[0]);
        return new DataFrame(data);
    }
    ...
    merge(frame) {
        ...
        const includes = this.rows.reduce((acc, curr) => {
            acc[curr[0].toString()] = typeof keys2[curr[0].toString()] === 'number';
            return acc;
        }, {});

        const rows = this.rows.map((element) => {
            const key = element[0];
            const has = typeof keys2[key.toString()] === 'number';
            return [key, element[1].concat(has ? frame.rows[keys2[key.toString()]][1] : emptyColumns[1])];
        });

        const additional = frame.rows.reduce((acc, element) => {
            const key = element[0];
            if (!includes[key.toString()]) {
                acc.push([key, emptyColumns[0].concat(element[1])]);
            }
            return acc;
        }, []);

        data.rows = rows.concat(additional);
        return new DataFrame(data);
    }
    ...
    _mergeMeta(frame) {
        const meta = {
            keys: this._lookupCells(this.meta.keys, null),
            columns: this.meta.columns.map((element) => `$1.${element}`)
                .concat(frame.meta.columns.map((element) => `$2.${element}`))
        };
